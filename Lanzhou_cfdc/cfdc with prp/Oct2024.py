### INP浓度和降水量的时间序列 (2024.10)
### 由于2024年10月份的数据文件编码问题, 需要指定 encoding='utf-8', encoding_errors='ignore' 来正确读取文件.与其他月份的代码略有区别.
...
for i, temp in enumerate(temperature):
    ...
    for file in p.glob("data_202410*.csv"): #### 修改此处文件名可选择不同时间段的数据 ####
    ...
    data_prp_selected = data_prp[(date >= '2024-10-01') & (date < '2024-11-01')]
    ### 读取日期数据, 为绘图做准备
    x_inp = df_inp['Date']
    ...
    ax1.set_xlabel('Date in October 2024')###
    ### y轴的设置
...
### 保存图片
plt.savefig(r"D:\Coding\master0_2025\Lanzhou_cfdc\202410.png")###