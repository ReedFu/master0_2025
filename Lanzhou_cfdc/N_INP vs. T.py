# -*- coding: utf-8 -*-
# 将全年数据输出为单个CSV文件， 并绘制INP浓度与温度的关系图
...
def main():
    ...
    for temp in TEMPERATURE:
        ...
        for file in CFDC_PATH.glob("*.csv"): 
            ...
            #### 确定日期
            t = pd.to_numeric(data['Time'], errors='coerce')# 把非数字变成 NaN (这行用于处理非utf-8编码时出现的异常)
            dt = base_date + pd.to_timedelta(t, unit='s')
            ...
            T_inp = data['Lamina Average T [C]'][sam_toggle==True][data['Lamina T Set [C]']==temp].reindex(dt.index, fill_value=np.nan)
            SS_w = data['Lamina SS_w'][sam_toggle==True][data['Lamina T Set [C]']==temp].reindex(dt.index, fill_value=np.nan)
            SS_i = data['Lamina SS_i'][sam_toggle==True][data['Lamina T Set [C]']==temp].reindex(dt.index, fill_value=np.nan)
            #### 计算净INP浓度
    ...
    df_inp.to_csv(r"D:\Coding\master0_2025\Lanzhou_cfdc\N_INP(202409-202509).csv", index=False)

    x_inp = df_inp['T_inp(°C)']
    y_inp = df_inp['N_inp_net(#/L)']

    fig = plt.figure(dpi=1000, figsize=(16, 8))
    ax = fig.add_subplot(111)

    ax.scatter(x_inp, y_inp, s=10, color='blue', alpha=0.5, label='INP Concentration vs Temperature')
    ax.set_xlabel("Temperature (°C)")
    ax.set_ylabel("INP Concentration (#/L)")
    ax.set_yscale('log')
    plt.savefig(r"D:\Coding\master0_2025\Lanzhou_cfdc\N_INP vs T.png")
