...
START_TIME = pd.to_datetime("2026-05-22 00:00:00") # 数据处理的起始时间, 包含在内
END_TIME = pd.to_datetime("2026-05-23 00:00:00") # 数据处理的结束时间, 包含在内
CFDC_PATH = Path(r"D:\Coding\Data\CFDC_train_video") # CFDC数据文件(csv格式)所在路径, 不包含子文件夹
...
def main():
    ...
    for temp in TEMPERATURE:
        ...
        for file in CFDC_PATH.glob("*.csv"): 
            ...
            if not (START_TIME <= base_date <= END_TIME):
                continue
            # 只读取前272列；遇到异常列数的坏行时跳过，避免 ParserError 中断流程
            data = pd.read_csv(
                file,
                encoding='utf-8',
                ...
                on_bad_lines='skip',
                engine='python'
            ).dropna(subset=['Lamina Average T [C]'])
            if data.empty:
                continue
            #### 确定日期
    ...
    # 输出为CSV文件
    df_inp.to_csv(r"D:\Coding\Data\CFDC_train_video\processed\N_INP.csv", index=False)
